...
- Added `src/madsci_client/madsci/client/data_client.py` to the `madsci-logging-patterns` pre-commit hook scope.
- Audited DataClient logging to ensure structured logging (no f-strings) and `event_type` on EventClient-style log calls.
