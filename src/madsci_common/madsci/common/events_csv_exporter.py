...
class CSVExporter:
    ...
    def export_utilization_periods_to_csv(
        ...
        writer.writerow(["Peak Utilization (%)", key_metrics.get("peak_utilization", "")])
        ...
        writer.writerow([
            "Period Number", "Period Type", "Period Display", "Date", 
            "Start Time", "Utilization (%)", "Experiments", "Runtime (hours)"
        ])
        ...
        if user_utilization:
            ...
            writer.writerow([
                "Author", "Total Workflows", "Total Runtime (hours)", 
        ...
        if experiment_details and experiment_details.get("experiments"):
            ...
            writer.writerow([
                "Experiment ID", "Experiment Name", "Start Time", "End Time", 
                "Status", "Duration (hours)", "Duration Display"
            ])
        ...
        if output_path:
            return CSVExporter._save_single_csv_file(
                csv_content, output_path, "utilization_periods", 
    ...
    def export_user_utilization_to_csv(
        ...
        if detailed:
            ...
            writer.writerow(["Total Workflows", system_summary.get("total_workflows", "")])
            writer.writerow(["Total Runtime (hours)", system_summary.get("total_runtime_hours", "")])
            writer.writerow(["Average Workflow Duration (hours)", system_summary.get("average_workflow_duration_hours", "")])
            writer.writerow(["Completion Rate (%)", system_summary.get("completion_rate_percent", "")])
            writer.writerow(["Author Attribution Rate (%)", system_summary.get("author_attribution_rate_percent", "")])
            writer.writerow([])
            
            # User details
            writer.writerow(["=== USER DETAILS ==="])
            writer.writerow([
                "Author", "Total Workflows", "Completed", "Failed", "Cancelled",
                "Total Runtime (hours)", "Avg Duration (hours)", "Completion Rate (%)",
                "Shortest (hours)", "Longest (hours)"
            ])
            ...
            writer.writerow(["Author Attribution Rate (%)", report_data.get("author_attribution_rate_percent", "")])
            writer.writerow([])
            
            # System totals
            system_totals = report_data.get("system_totals", {})
            writer.writerow(["System Totals"])
            writer.writerow(["Total Workflows", system_totals.get("total_workflows", "")])
            writer.writerow(["Total Runtime (hours)", system_totals.get("total_runtime_hours", "")])
            writer.writerow(["Completion Rate (%)", system_totals.get("completion_rate_percent", "")])
            writer.writerow([])
            ...
            writer.writerow([
                "Author", "Total Workflows", "Total Runtime (hours)", 
                "Completion Rate (%)", "Avg Duration (hours)"
            ])
    ...
    def export_time_series_to_csv(
        ...
        """
        Export time series analysis to a single CSV.
        
        Args:
            report_data: The time series report data
            output_path: Optional path to save CSV file
        ...
        # Use summary data if available, otherwise use detailed
        summary_data = report_data.get("summary", report_data)
        ...
        # System time series
        writer.writerow(["=== SYSTEM TIME SERIES ==="])
        writer.writerow([
            "Period Number", "Period Type", "Period Display", "Date", 
            "Start Time", "Utilization (%)", "Experiments", "Runtime (hours)"
        ])
        
        time_series = summary_data.get("time_series", {}).get("system", [])
        for point in time_series:
            writer.writerow([
                point.get("period_number", ""),
                point.get("period_type", ""),
        ...
        if trends:
            ...
            util_trend = trends.get("utilization_trend", {})
            exp_trend = trends.get("experiment_trend", {})
            
            writer.writerow(["Utilization Trend"])
            writer.writerow(["Direction", util_trend.get("direction", "")])
            writer.writerow(["Change (%)", util_trend.get("change_percent", "")])
            writer.writerow(["First Value", util_trend.get("first_value", "")])
            writer.writerow(["Last Value", util_trend.get("last_value", "")])
            writer.writerow([])
            
            writer.writerow(["Experiment Trend"])
            writer.writerow(["Direction", exp_trend.get("direction", "")])
            writer.writerow(["Change (%)", exp_trend.get("change_percent", "")])
            writer.writerow(["First Value", exp_trend.get("first_value", "")])
            writer.writerow(["Last Value", exp_trend.get("last_value", "")])
        ...
        if output_path:
            return CSVExporter._save_single_csv_file(
                csv_content, output_path, "time_series", 
                metadata.get("analysis_type", "daily")
            )
    ...
    def export_utilization_report_to_csv(
        ...
        writer.writerow(["Analysis Duration (hours)", metadata.get("analysis_duration_hours", "")])
        writer.writerow([])
        ...
        writer.writerow(["Total System Runtime (hours)", overall_summary.get("total_system_runtime_hours", "")])
        writer.writerow(["Total Active Time (hours)", overall_summary.get("total_active_time_hours", "")])
        writer.writerow(["Average System Utilization (%)", overall_summary.get("average_system_utilization_percent", "")])
        writer.writerow(["Total Experiments", overall_summary.get("total_experiments", "")])
        ...
        if session_details:
            ...
            writer.writerow([
                "Session Type", "Session Name", "Start Time", "End Time",
                "Duration (hours)", "System Utilization (%)", "Total Experiments", "Nodes Active"
            ])
        ...
        if node_summary:
            writer.writerow(["=== NODE SUMMARY ==="])
            writer.writerow([
                "Node ID", "Average Utilization (%)", "Total Busy Time (hours)", "Sessions Active"
            ])
