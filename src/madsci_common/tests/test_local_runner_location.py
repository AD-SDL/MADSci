...
def test_local_runner_location_manager_has_both_handlers():
    """Verify LocalRunner creates a mongo handler for the location manager."""
    runner = LocalRunner()
    assert hasattr(runner, "_location_mongo_handler")
    assert runner._location_mongo_handler is not None
