...
class TransferGraph:
    ...
    def find_shortest_path(self, source: str, target: str) -> Optional[List[Dict]]:
        ...
        if source not in self.locations or target not in self.locations:
            print(f"Debug: Source or target not in locations. Source in: {source in self.locations}, Target in: {target in self.locations}")
            return None
        
        print(f"Debug: Available edges from source '{source}': {self.edges.get(source, [])}")
        ...
        while pq:
            ...
            if current == target:
                print(f"Debug: Found target! Reconstructing path...")
                # Reconstruct path
                path = []
                node = target
            ...
            # Check all neighbors
            print(f"Debug: Checking neighbors of '{current}': {len(self.edges[current])} edges")
            for neighbor, robot, edge_cost in self.edges[current]:
                print(f"Debug: Edge: {current} -> {neighbor} via '{robot}' (cost: {edge_cost})")
    ...
    def find_all_paths(self, source: str, target: str, max_length: int = 5) -> List[List[Dict]]:
