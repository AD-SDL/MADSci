...
class TransferManagerConfig(BaseModel):
    ...
    def __init__(self, config_file_path: Optional[Path] = None, **data: Any) -> None:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        with Path.open(self.config_file_path) as f:
...
class TransferManager:
    ...
    def _load_configuration(self) -> None:
        ...
        if hasattr(self.config, "load_config"):
            ...
            with Path.open(self.config.robot_definitions_path) as f:
                self.robot_definitions = yaml.safe_load(f)

            with Path.open(self.config.location_constraints_path) as f:
                self.location_constraints = yaml.safe_load(f)

        self.logger.info(
            f"Loaded {len(self.robot_definitions)} robot definitions: {list(self.robot_definitions.keys())}"
        )
        self.logger.info(
            f"Loaded {len(self.location_constraints)} location constraints: {list(self.location_constraints.keys())}"
        )
    ...
    def _build_transfer_graph(self) -> None:
        ...
        total_edges = 0
        for robot_name, _robot_def in self.robot_definitions.items():
    ...
    def _calculate_transfer_cost(
        ...
        complexity = len(source_constraint.get("default_args", {})) + len(
            target_constraint.get("default_args", {})
        )
    ...
    def expand_transfer_step(self, step: Step) -> List[Step]:
        ...
        if step.node and step.node.strip() and step.node in self.robot_definitions:
            self.logger.info(
                f"Checking if robot '{step.node}' can do direct transfer..."
            )
            ...
            self.logger.warning(
                f"Robot '{step.node}' cannot do direct transfer - using graph"
            )
        else:
        ...
        for i, hop in enumerate(transfer_path):
            self.logger.info(
                f"  Hop {i + 1}: {hop['source']} -> {hop['target']} via '{hop['robot']}'"
            )
        ...
        for i, hop in enumerate(transfer_path):
            ...
            concrete_step.name = (
                f"Transfer {i + 1}: {hop['source']} -> {hop['target']} via {robot_name}"
            )
    ...
    def _build_transfer_step(
        ...
        if original_locations:
            ...
            for _key, location_obj in original_locations.items():
                location_name = self._extract_location_name(location_obj)
                if location_name == source:
                    updated_location = self._update_location_with_lookup(
                        location_obj, source, robot_name
                    )
                    step_locations["source"] = updated_location
                elif location_name == target:
                    updated_location = self._update_location_with_lookup(
                        location_obj, target, robot_name
                    )
            ...
            if "source" not in step_locations:
                step_locations["source"] = self._create_location_argument_with_lookup(
                    source, robot_name
                )
            if "target" not in step_locations:
                step_locations["target"] = self._create_location_argument_with_lookup(
                    target, robot_name
                )
    ...
    def _update_location_with_lookup(
        self, location_obj: Any, location_name: str, robot_name: str
    ) -> Any:
        ...
        if lookup_coordinates is not None:
            ...
            self.logger.warning(
                f"No lookup coordinates found for {location_name} with robot {robot_name}"
            )

        return updated_location

    def _create_location_argument_with_lookup(
        self, location_name: str, robot_name: str
    ) -> Dict:
    ...
    def _get_lookup_coordinates(self, location_name: str, robot_name: str) -> Any:
        ...
        self.logger.warning(
            f"No lookup coordinates found for {location_name}.{robot_name}"
        )
        return None
    ...
    def _extract_location_name(
        self, location_value: Union[LocationArgument, str]
    ) -> str:
        """
        ...
        if hasattr(location_value, "location_name") or hasattr(
            location_value, "location"
        ):
            # First try location_name if it's provided and not None
            if (
                hasattr(location_value, "location_name")
                and location_value.location_name
            ):
                return location_value.location_name

            # If location_name is None/missing, use location field as string
            if (
                hasattr(location_value, "location")
                and location_value.location is not None
            ):
    ...
    def _replace_template_variables(self, template: Any, **variables: Any) -> Any:
    ...
    def get_transfer_options(self, source: str, target: str) -> List[Dict]:
