...
class TransferGraph:
    ...
    def add_edge(self, source: str, target: str, robot: str, cost: float = 1.0) -> None:
        """Add a transfer edge between two locations."""
        if not robot or not robot.strip():
            self.logger.error(
                f"ERROR: Cannot add edge with empty robot: {source} -> {target}"
            )
            return
    ...
    def find_shortest_path(self, source: str, target: str) -> Optional[List[Dict]]:
        ...
        if source not in self.locations:
            self.logger.warning(
                f"Source '{source}' not in graph locations: {sorted(self.locations)}"
            )
            return None
        if target not in self.locations:
            self.logger.warning(
                f"Target '{target}' not in graph locations: {sorted(self.locations)}"
            )
            return None
        ...
        while pq:
            ...
            if current == target:
                self.logger.info("Found target! Reconstructing path...")
                # Reconstruct path
                path = []
                node = target
            ...
            for neighbor, robot, edge_cost in self.edges[current]:
                self.logger.info(
                    f"Checking edge: {current} -> {neighbor} via '{robot}' (cost: {edge_cost})"
                )
