...
The SilaNodeClient SHALL declare `url_protocols = ["sila"]` so that the existing `find_node_client()` dispatch mechanism routes `sila://` URLs to it.

#### Scenario: URL validation accepts sila scheme
- **WHEN** `SilaNodeClient.validate_url()` is called with a `sila://host:port` URL
- **THEN** it SHALL return `True`

#### Scenario: URL validation rejects non-sila schemes
- **WHEN** `SilaNodeClient.validate_url()` is called with an `http://` or `https://` URL
